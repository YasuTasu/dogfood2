    ...
    if request.method == 'GET':
        try:
            ...
            if not selected_list:
                ...
                selected_list = [
                    {'food_code': ing.food_code, 'grams': item['grams'], 'name': ing.name}
                    for item in selected_list for ing in ingredients if int(item['food_code']) == ing.food_code
                ]

            # 全食材リストを取得（セレクトボックス用）
            all_ingredients = Ingredient.query.all()
            available_ingredients = [
                {'food_code': ing.food_code, 'name': ing.name} for ing in all_ingredients
            ]
            ...
            deficiencies = [nutrient for nutrient, total in nutrient_totals.items() if total < aafco_standards[nutrient]]
            suggestions = suggest_ingredients_for_deficiencies(deficiencies)
            ...
            response_data = {
                ...
                "deficiencies": deficiencies,
                "suggestions": suggestions,
                "available_ingredients": available_ingredients,  # 追加: 全食材リスト
                "result_symbols": {
